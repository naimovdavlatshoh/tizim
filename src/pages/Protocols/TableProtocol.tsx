...
export default function TableProtocol({
  ...
  const handleDownloadWord = async (protocol: Protocol) => {
    ...
    try {
      ...
      let fileName = `protocol_${protocol.protocol_id}.doc`;
      ...
      toast.error(error?.response?.data?.error || "Ошибка скачивания файла");
    } finally {
  ...
  if (protocols.length === 0) {
    return (
      <div className="text-center py-8 text-gray-500">Протоколы не найдены</div>
    );
  }
    ...
    <>
              ...
              >
                ...
                <td className="px-2 sm:px-4 py-2 sm:py-3 text-sm text-gray-900 dark:text-gray-100 overflow-visible text-center">
                  {protocol.sender_comments ? (
                    <button
                      onClick={() => openCommentModal(protocol.sender_comments)}
                ...
                <td className="pl-2 sm:pl-4 pr-3 sm:pr-5 py-2 sm:py-3 text-sm text-gray-900 dark:text-gray-100">
                  <div className="flex flex-row items-center gap-2 flex-nowrap">
                    <div title="QR-код">
                      <Button
                        onClick={() => handleGetQrCode(protocol)}
                        size="xs"
                        variant="outline"
                      >
                        <svg
                    ...
                    {protocol.is_word_added === 1 ? (
                      ...
                      <div title="Скачать Word файл">
                        <Button
                          ...
                          onClick={() => handleDownloadWord(protocol)}
                          size="xs"
                      ...
                      <>
                        <div title="Загрузить Word файл">
                          <Button
                            onClick={() => openWordUploadModal(protocol)}
                            size="xs"
                            variant="primary"
                          >
                            <svg
                    ...
                    <div title="Загрузить PDF файл">
                      <Button
                        onClick={() => openPdfUploadModal(protocol)}
                        size="xs"
                    ...
                    <div title="Скачать PDF файл">
                      <Button
                        ...
                        size="xs"
                        disabled={downloadingPdf || protocol.is_pdf_added !== 1}
                      >
                        <svg
                          className="w-4 h-4"
                          fill="none"
      ...
      >
        <div className="p-6 pt-10 sm:pt-16">
          <h3 className="text-lg font-semibold mb-4 text-gray-900 dark:text-white">
            QR-код протокола
            {selectedProtocolForQr?.protocol_number !== undefined && (
      ...
      >
        <div className="p-6 pt-10 sm:pt-16">
          <h3 className="text-lg font-semibold mb-4 text-gray-900 dark:text-white">
            Загрузить Word файл
            {selectedProtocolForWord?.protocol_number !== undefined && (
              <span className="block text-sm font-normal text-gray-500 dark:text-gray-400 mt-1">
                Номер протокола: {selectedProtocolForWord.protocol_number}
              </span>
            )}
          </h3>
      ...
      >
        <div className="p-6 pt-10 sm:pt-16">
          <h3 className="text-lg font-semibold mb-4 text-gray-900 dark:text-white">
            Загрузить PDF файл
            {selectedProtocolForPdf?.protocol_number !== undefined && (
              <span className="block text-sm font-normal text-gray-500 dark:text-gray-400 mt-1">
                Номер протокола: {selectedProtocolForPdf.protocol_number}
              </span>
            )}
          </h3>
