...
export default function AddMixModal({
    isOpen,
    onClose,
    changeStatus,
}: AddMixModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            const response: any = await PostSimple("api/mix/create", {
            ...
            if (response) {
                toast.success("Смесь успешно создана");
                setFormData({
                    client_id: 0,
                    category_name: "",
                    application_date: "",
            ...
            toast.error(
                error?.response?.data?.error ||
                    "Что-то пошло не так при создании смеси"
            );
        } finally {
            setLoading(false);
        }
