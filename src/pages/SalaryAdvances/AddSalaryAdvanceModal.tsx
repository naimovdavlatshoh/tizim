...
export default function AddSalaryAdvanceModal({
    ...
    const handlePriceChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setFormData({
            ...formData,
            amount_of_advance: numericValue,
        });
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (
            !formData.to_user_id ||
            !formData.advance_comments ||
            !formData.amount_of_advance
        ) {
        ...
        try {
            const response: any = await PostSimple(
                "api/user-salary-advance/create",
                {
                    to_user_id: Number(formData.to_user_id),
                    advance_comments: formData.advance_comments,
                    amount_of_advance: Number(formData.amount_of_advance),
                }
            );

            if (response) {
                toast.success("Аванс успешно создан");
                setFormData({
                    to_user_id: "",
                    advance_comments: "",
                    amount_of_advance: "",
            ...
            toast.error(
                error?.response?.data?.error ||
                    "Что-то пошло не так при создании аванса"
            );
        } finally {
            setLoading(false);
        }
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="grid grid-cols-1 gap-6">
                        ...
                        <div>
                            ...
                            <InputField
                                type="text"
                                value={formData.advance_comments}
                                onChange={(e) =>
                                    setFormData({
                                        ...formData,
                                        advance_comments: e.target.value,
                                    })
                                }
                                placeholder="Введите комментарий к авансу"
