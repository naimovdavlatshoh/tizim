...
export default function AddFineModal({
    ...
    const handlePriceChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setFormData({
            ...formData,
            amount_of_fine: numericValue, // Store without spaces for API
        });
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (
            !formData.to_user_id ||
            !formData.fine_comments ||
            !formData.amount_of_fine
        ) {
        ...
        setLoading(true);
        try {
            const response: any = await PostSimple("api/user-fines/create", {
                to_user_id: Number(formData.to_user_id),
                fine_comments: formData.fine_comments,
                amount_of_fine: Number(formData.amount_of_fine),
            });

            if (response) {
                toast.success("Штраф успешно создан");
                setFormData({
                    to_user_id: "",
                    fine_comments: "",
                    amount_of_fine: "",
            ...
            toast.error(
                error?.response?.data?.error ||
                    "Что-то пошло не так при создании штрафа"
            );
        } finally {
            setLoading(false);
        }
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="grid grid-cols-1 gap-6">
                        ...
                        <div>
                            ...
                            <InputField
                                type="text"
                                value={formData.fine_comments}
                                onChange={(e) =>
                                    setFormData({
                                        ...formData,
                                        fine_comments: e.target.value,
                                    })
                                }
                                placeholder="Введите комментарий к штрафу"
